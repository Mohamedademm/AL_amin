...
- `server/`: Backend (Node + Express 5 + Prisma 7 + PostgreSQL)
  - `src/`
    ...
    - `modules/`: Domain modules, each with `routes` / `controller` / `service` (`auth`, `product`, `category`, `order`, `inventory`, `spot`, `user`, `dashboard`, `discount`, `audit`, `restock` — predictive replenishment forecast, `loyalty` — gamified tier status; `order` also has `cleanup.ts` — interval-based order expiry, started by the local server only)
