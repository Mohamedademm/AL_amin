...
export default function Inventory() {
  ...
  const columns: Column<InventoryRecord>[] = [
    ...
    },
    {
      key: "spot",
      header: "Spot",
      sortValue: (r) => r.spot?.name ?? "",
      render: (r) => <span className="text-muted">{r.spot?.name}</span>,
    },
    {
      key: "status",
      header: "Status",
      ...
      render: (r) => (
        <Badge tone={r.quantity < LOW ? "amber" : "success"}>
          {r.quantity < LOW ? "Low" : "In stock"}
        </Badge>
      ),
    },
    {
      ...
      render: (r) => {
        ...
        return (
          <input
            ...
            className={cn(
              "w-20 rounded-lg border bg-surface px-2 py-1.5 text-right outline-none focus:border-primary",
              dirty ? "border-primary" : "border-line",
            )}
          />
        );
      },
  ...
  const rows = useMemo(
    () => (inventory ?? []).filter((r) => !lowOnly || r.quantity < LOW),
    [inventory, lowOnly],
  );
